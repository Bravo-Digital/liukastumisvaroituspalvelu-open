...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build where conditions - always include time filter for future warnings
    const whereConditions = [gt(warningsTable.onsetAt, now)];
    ...
    console.error('Error fetching future warnings:', error);
    return NextResponse.json(
      { error: 'Failed to fetch future warnings' },
      { status: 500 }
    );
  }
}